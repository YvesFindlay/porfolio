...
const Skills = () => {
  const skills = ["JavaScript", "React", "HTML5", "Sass", "CSS3", "Bootstrap", "Figma", "GSAP3", "npm"];
  const skillElems = skills.map((skill, ind)=> {
    return (
      <div key={ind} className="skills__item">{skill}</div>
    )
  })
  ...
  useEffect(() => {
    ...
    expandLineAnimation({
      ...
      delay: 0.5,
    }).to('.skills__item', {opacity: 1, ease: Power1.easeInOut}, ">").add(marquee(no01, 20, fromLeft));
  }, []);
